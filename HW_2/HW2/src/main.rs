...
fn test_decl(score: f64, input: String, out: Option<asgn2::Decl>) -> f64 {
    match (asgn2::parser::decl(&input), out) {
        (Ok(d1), Some(d2)) => {
            if asgn2::decl_eq(d1.clone(), d2.clone()) {
                return score;
            } else {
                println!(
...
pub fn main() {
    ...
    let r22 = test_expr(3.0, "234".to_string(), Some(ex::Numeral(234.0)));
    let r23 = test_expr(
        3.0,
        "x*y".to_string(),
        Some(ex::Times(
    ...
    let r24 = test_expr(
        ...
        "1.2+z_3".to_string(),
        Some(ex::Plus(
            Box::new(ex::Numeral(1.2)),
            Box::new(ex::Id("z_3".to_string())),
        )),
    );
    let r25 = test_expr(
        3.0,
        "3-1".to_string(),
        Some(ex::Minus(
            Box::new(ex::Numeral(3.0)),
            Box::new(ex::Numeral(1.0)),
        )),
    );
    let r26 = test_expr(
        3.0,
        "(2-(y))".to_string(),
        Some(ex::Minus(
            Box::new(ex::Numeral(2.0)),
            Box::new(ex::Id("y".to_string())),
        )),
    );
    let r27 = test_expr(3.0, "((2-(y))".to_string(), None);
    let r28 = test_expr(3.0, "+32".to_string(), None);
    let r29 = test_expr(
        3.0,
        "1*2+3*4".to_string(),
        Some(ex::Plus(
    ...
    let r34a = test_expr(
        3.0,
        "let function f(x){y*x} in z".to_string(),
        Some(ex::Let(
            Box::new(dc::FunDecl(
                "f".to_string(),
                vec!["x".to_string()],
    ...
    let r34b = test_expr(
        3.0,
        "let function f(x){y*x} in f(2)".to_string(),
        Some(ex::Let(
            Box::new(dc::FunDecl(
                "f".to_string(),
                vec!["x".to_string()],
    ...
    let r35 = test_expr(
        2.0,
        "let var x = let var y = 1 in y in let var y = x in x".to_string(),
        Some(ex::Let(
            Box::new(dc::VarDecl(
                "x".to_string(),
                Box::new(ex::Let(
                    Box::new(dc::VarDecl("y".to_string(), Box::new(ex::Numeral(1.0)))),
                    Box::new(ex::Id("y".to_string())),
                )),
            )),
            Box::new(ex::Let(
    ...
    let r36 = test_expr(
        3.0,
        "(let var x = y in x)+(let var x = y in x)".to_string(),
        Some(ex::Plus(
            Box::new(ex::Let(
                Box::new(dc::VarDecl(
                    "x".to_string(),
    ...
    let r39 = test_decl(
        3.0,
        "var x = let var y = 1 in y".to_string(),
        Some(dc::VarDecl(
            "x".to_string(),
            Box::new(ex::Let(
                Box::new(dc::VarDecl("y".to_string(), Box::new(ex::Numeral(1.0)))),
